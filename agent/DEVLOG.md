...
- Added no-slots escalation to `appointmentAvailabilityTool`: when all queried provider/date combinations return zero available slots, agent sets `escalated=true` and creates a high-urgency OpenEMR task via the existing `createEscalationTask` path (same mechanism as severe drug interaction escalation)
- Added voice-to-text (STT) to the AI assistant panel using the browser Web Speech API: mic button starts continuous recognition with real-time interim results shown in the input, auto-sends after a 1s pause in speech, resets input after each send, and stays active until manually stopped
